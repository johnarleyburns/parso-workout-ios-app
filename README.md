...
- **Observations + personalized workouts** — Home keeps cited Observations separate from an on-demand, editable Personalized Workout chooser reached from Start Workout. It calculates Minimum (4), Medium (8), and Maximal (12) weekly-set targets from direct/indirect muscle work, shows remaining gaps and safety-cap trims, and never replaces the user's decision to train. Custom and personalized workouts can be scheduled as one-off local date/time snapshots; the retired weekly Plan tab is not part of the primary flow.
...
- **Privacy-first, synced** — SwiftData with no accounts, no third-party server, no ads. Your full training log **syncs live across your iPhone and Apple Watch** through **your own private iCloud** (CloudKit) — never a Cladiron server, never seen by us — and Settings reports the Apple-ID/iCloud sync precondition instead of claiming sync is active when the account is unavailable. A complete, compressed JSON export (`.json.gz`, imports also accept plain `.json`) still lets you back up and move your data (full history + preferences) off-platform.
...
`bash scripts/install-git-hooks.sh`: pre-commit runs only
`swift test --package-path CadenceCore` and never boots a simulator. Pre-push
runs no tests. Run the simulator smoke suite explicitly with `make all-tests`
before a release, TestFlight submission, or field-testing build; the smoke
suite remains a release gate rather than a commit-time action.

**Timing notes:** `swift test` takes ~50s and `xcodebuild build` ~20s. The
pre-commit hook is intentionally host-only; allow at least 300s when scripting
`git commit` for the package suite. The pre-release smoke suite can take longer
because it builds and launches the pinned iPhone and Watch targets. CI workflows
