...
| Rich per-set intent and source identity into the live session | **partial → adapter slice landed** | `PlannedSetPrescription` now carries optional source set ID, load mode, `%1RM`, RPE, rest, and warm-up; `WorkoutSession` carries optional `planSessionID`; `RuntimePrescriptionAdapterTests` prove resolution, stable IDs, all item-family conversion, and legacy fields. The repository now exposes a `Session`-based iPhone materializer; actual plan production/start UI and plan provenance beyond the session are still open. |
...
2. **Promote the adapter to a single materializer entry point** used by the
   ...
   and migration coverage prove equivalent output.
