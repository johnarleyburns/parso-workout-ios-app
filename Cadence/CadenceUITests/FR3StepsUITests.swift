...
final class FR3StepsUITests: CadenceUITestCase {
    ...
    func testGoalRingAndTrend() {
        ...
        app.goToTab("Today")
        XCTAssertTrue(app.otherElements["today.goalRing"].waitForExistence(timeout: 10),
                      "step goal ring should be visible")
        XCTAssertTrue(app.otherElements["today.trendChart"].waitForExistence(timeout: 10)
