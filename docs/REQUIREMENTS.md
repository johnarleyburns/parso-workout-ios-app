...
**Status:** Draft v0.2 · **Platforms:** watchOS 10+ (watch-first for workouts) & iOS 17+ (review hub + watchless logging) · **License:** MIT (proposed) · **Distribution:** Source + TestFlight
...
- **Shared core (foundation):** a Swift package with the data model, Smart Start ranking, PR logic, and the CloudKit sync layer — consumed by both targets. Watch-first front-loads this; it's prerequisite to P1.
- **P1 MVP (watch-first logging):** FR-1, **FR-8.1/8.2 (wrist strength logging)**, **FR-9 (sync + roles)**, FR-3, FR-2.1, FR-4.1/4.2, UC-1/6/10.
- **P2:** **FR-8.3/8.4 (watch live cardio + wrist Smart Start)**, FR-5, FR-4.3, FR-7 baseline, UC-2/7/8/9/11.
- **P3:** FR-2.2–2.5, FR-4.4, FR-7 enhancements (FTMS/NFC/beacon), FR-8.6 complications, UC-3/4/5 (GPS + chest strap).
