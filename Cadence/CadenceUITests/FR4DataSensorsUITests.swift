...
final class FR4DataSensorsUITests: CadenceUITestCase {
    ...
    func testHealthPriming() {
        ...
        XCTAssertTrue(app.staticTexts["Connect Apple Health"].waitForExistence(timeout: 10),
    ...
    func testHRMPairing() {
        ...
        XCTAssertTrue(connect.waitForExistence(timeout: 10), "discovered device should appear")
        connect.tap()
        // A live BPM readout in the "My Device" section proves it connected.
        let bpm = app.staticTexts["hrm.bpm"]
        XCTAssertTrue(bpm.waitForExistence(timeout: 12), "live BPM should appear after connect")
    ...
    func testSaveStrengthToHealth() {
        ...
        XCTAssertTrue(app.staticTexts["session.healthSaved"].waitForExistence(timeout: 10),
